...
import { getHandlerById, handlerIds, handlers } from '../handlers/index.js';
...
export const onData = (socket) => async (data) => {
  ...
  while (socket.buffer.length >= PACKET_TOTAL_LENGTH) {
    ...
    if (socket.buffer.length >= length) {
      // 패킷 데이터를 자르고 버퍼에서 제거
      const packet = socket.buffer.slice(PACKET_TOTAL_LENGTH, length);
